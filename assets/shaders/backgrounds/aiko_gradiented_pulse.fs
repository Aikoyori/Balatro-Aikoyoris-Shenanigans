...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    */
    ...
    PREC number colour1_angles = 1. - fract ( mod ( ( (  -polarcoord.y + (spin_time + ( spumamnt * 0.50) ) * 0.01 + (emit) * 0.15) / (PI / 06.) + time * 0.023  ) , 2.0 ) ) ;
    PREC number colour2_angles = 1. - fract ( mod ( ( (  -polarcoord.y + (spin_time + ( spumamnt * 1.50) ) * 0.07 + (time) * 0.30) / (PI / 04.) + time * 0.041  ) , 2.0 ) ) ;
    PREC number colour3_angles = 1. - fract ( mod ( ( (  -polarcoord.y + (spin_time + (-spumamnt * 2.75) ) * 0.04 + (emit) * 0.07) / (PI / 22.) + emit * 0.069  ) , 2.0 ) ) ;
