...
function buildAdditionsTab(mod)
        ...
        tab_definition_function = function()
            ...
            local consumables_alphabet = AKYRS.card_collection_ui_template('c_akyrs_alphabet_a')
...
AKYRS.enchantment_collection_UIBox = function(_pool, rows, args)
    ...
    local deck_tables = {}
    local pool_fake = SMODS.collection_pool(_pool)
    local pool = {}
    local sortorder = 0
    for i, _c in ipairs(pool_fake) do
        for i = 1, _c.max_level do
            pool[#pool+1] = {_c, i, sortorder}
    ...
    G.FUNCS.AKYRS_enchanted_collection_page = function(e)
        ...
        for j = 1, #rows do
            for i = 1, rows[j] do
                local data = pool[i+row_totals[j] + (cards_per_page*(e.cycle_config.current_option - 1))]
                ...
                card.ability.akyrs_level = data[2]
    ...
    G.FUNCS.AKYRS_enchanted_collection_page{ cycle_config = { current_option = 1 }}
    ...
    local t = create_UIBox_generic_options({
        ...
        back_func = (args and args.back_func) or G.ACTIVE_MOD_UI and "openModUI_"..G.ACTIVE_MOD_UI.id or 'your_collection', snap_back = args.snap_back, infotip = args.infotip, contents = {
          ...
          (not args.hide_single_page or cards_per_page < #pool) and {n=G.UIT.R, config={align = "cm"}, nodes={
            create_option_cycle({options = options, w = 4.5, cycle_shoulders = true, opt_callback = 'AKYRS_enchanted_collection_page', current_option = 1, colour = G.ACTIVE_MOD_UI and (G.ACTIVE_MOD_UI.ui_config or {}).collection_option_cycle_colour or G.C.RED, no_pips = true, focus_args = {snap_to = true, nav = 'wide'}})
          }} or nil,
      }})
    return t
end
...
G.FUNCS.akyrs_your_collection_credits = function(e)
    G.SETTINGS.paused = true
    G.FUNCS.overlay_menu{
...
function G.UIDEF.card_focus_ui(card)
    ...
    local no_more_attach = false
    --The card UI can have BUY, REDEEM, USE, and SELL buttons depending on the context of the card
    if card.area == G.akyrs_life_shop and G.akyrs_life_shop and not AKYRS.not_buy_or_use_set[card.ability.set] then --Add a buy button
        ...
        base_attach.children.buy_and_use = G.UIDEF.card_focus_button{
            ...
            func = 'akyrs_can_buy_and_use', button = 'akyrs_use_card', card_width = card_width
        }
        buy_and_use = true
        end
        base_attach.children.buy = G.UIDEF.card_focus_button{
        card = card, parent = base_attach, type = 'buy',
        func = 'akyrs_can_buy', button = 'akyrs_buy_from_shop', card_width = card_width, buy_and_use = buy_and_use
        }
    end
    if card.area == G.akyrs_life_shop and G.akyrs_life_shop and AKYRS.redeemer_set[card.ability.set] then --Add a redeem button
        base_attach.children.redeem = G.UIDEF.card_focus_button{
        card = card, parent = base_attach, type = 'buy',
        func = 'akyrs_can_redeem', button = 'redeem_from_shop', card_width = card_width
        }
        no_more_attach = true
    end
    if card.area == G.akyrs_life_shop and G.akyrs_life_shop and AKYRS.opener_set[card.ability.set]  then --Add a redeem button
        base_attach.children.redeem = G.UIDEF.card_focus_button{
        card = card, parent = base_attach, type = 'buy',
        func = 'akyrs_can_open', button = 'open_booster', card_width = card_width*0.85
        }
        no_more_attach = true
    end
